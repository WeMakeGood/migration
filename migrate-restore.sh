...
while read APP SITE URL ARCHIVE PREFIX NEWSITE; do
	...
	if [ $SINGLE_DB == "" ]; then
		...
		# echo "$DB_PASSWORD $SITE_DB_NAME '{"name": "'"$SITE_DB_NAME"'", "password": "'"$DB_PASSWORD"'", "databases": '"$DB_USER_DBS"'}'"
		SITE_DB_USER="$(curl -v "${HEADERS[@]}" -X POST $API_URL/servers/$SERVER_ID/database-users -d '{"name": "'"$SITE_DB_NAME"'", "password": "'"$DB_PASSWORD"'", "databases": '"$DB_USER_DBS"'}' | jq -cr '.')"
	fi
