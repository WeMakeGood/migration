...
while read APP SITE URL ARCHIVE PREFIX NEWSITE; do
	...
	if [ $SINGLE_DB == "" ]; then
		...
		echo "$(curl "${HEADERS[@]}" https://httpbin.org/post -d '{"name":"'$SITE_DB_NAME'","password":"'$DB_PASSWORD'","databases": ['"$SITE_DB_ID"']}')"
		exit 99
		SITE_DB_USER="$(curl --trace-ascii "${HEADERS[@]}" -X POST $API_URL/servers/$SERVER_ID/database-users -d '{"name":"'$SITE_DB_NAME'","password":"'$DB_PASSWORD'","databases": '"$DB_USER_DBS"'}' | jq -cr '.')"
	fi
