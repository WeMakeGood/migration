...
while read APP SITE URL ARCHIVE PREFIX NEWSITE; do
	...
	if [ $SINGLE_DB == "" ]; then
		...
		SITE_DB_USER="$(curl --trace-ascii "${HEADERS[@]}" -X POST $API_URL/servers/$SERVER_ID/database-users -d '{"name":"'$SITE_DB_NAME'","password":"'$DB_PASSWORD'","databases": ['"$SITE_DB_ID"']}' | jq -cr '.')"
		echo "$SITE_DB_USER"
		exit 99
	fi
