...
SERVER_ID="$(curl -X GET $API_URL/servers -s | jq '.servers[] | select(.name="$SERVERNAME").id')"
SERVER_SITES="$(curl -X GET $API_URL/servers/$SERVER_ID/sites -s | jq -r '.sites[].name')"
DB_USER_ID="$(curl -X GET $API_URL/servers/$SERVER_ID/database-users -s | jq '.users[] | select(.name=='\"$USER\"').id')"

echo "$API_TOKEN"
echo "$(curl -X GET $API_URL/servers)"
...
while read APP SITE URL ARCHIVE PREFIX NEWSITE; do
	...
	SITE_DB_ID="$(curl -X GET $API_URL/servers/$SERVER_ID/databases | jq '.databases[] | select(.name=="'$SITE_DB_NAME'").id')"
	# Create a new database if it needs to be added
	if [ -v $SITE_DB_ID ]; then
		echo "Adding database..."
		# Create the database
		SITE_DB_ID="$(curl -X POST $API_URL/servers/$SERVER_ID/databases --data '{"name":"'"$SITE_DB_NAME"'"}' | jq '.database.id')"
	...
	DB_USER_DBS="$(curl -X GET $API_URL/servers/$SERVER_ID/database-users/$DB_USER_ID | jq -c '.user.databases')"
	if [[ ! "$DB_USER_DBS" == *"$SITE_DB_ID"* ]]; then
		DB_USER_DBS="$(echo "$DB_USER_DBS" | jq -c '. += ['$SITE_DB_ID'] | unique')"
		curl -X PUT $API_URL/servers/$SERVER_ID/database-users/$DB_USER_ID -d '{"databases": '"$DB_USER_DBS"'}' >/dev/null
	fi

	# Get the site ID or make it
	SITE_ID="$(curl -X GET $API_URL/servers/$SERVER_ID/sites | jq '.sites[] | select(.name=="'$SITE'").id')"
	if [ -v $SITE_ID ]; then
		echo "Creating site..."
		SITE_ID="$(curl -X POST $API_URL/servers/$SERVER_ID/sites -d '{"domain":"'"$SITE"'","project_type":"php","directory":"/public","isolated":true,"username":"'"$USER"'","php_version":"php80"}' | jq -cr '.site.id')"
		if [[ ! "$SITE_ID" =~ ^-?[0-9]+$ ]]; then
			echo $SITE_ID
			exit 99
		fi
	...
	if [ ! -f $SITE_ROOT/wp-config.php ]; then
		...
		curl -X POST $API_URL/servers/$SERVER_ID/sites/$SITE_ID/wordpress -d '{"database": "'"$SITE_DB_NAME"'", "user": '"$DB_USER_ID"'}' >/dev/null
